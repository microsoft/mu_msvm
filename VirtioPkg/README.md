...
The virtio-blk-over-PCI driver stack in this package (`VirtioPkg`) is **ported
verbatim from EDK2 OvmfPkg**. This document records exactly where the code came
from and what (if anything) was changed, so that:
...
`[Packages]` section from `OvmfPkg/OvmfPkg.dec` to `VirtioPkg/VirtioPkg.dec`. All
`.c` and `.h` files are **verbatim** copies of the upstream sources at the commit
above.

The `gVirtioDeviceProtocolGuid` GUID value is preserved from OvmfPkg (declared in
`VirtioPkg/VirtioPkg.dec`).
...
| VirtioPkg path | Upstream path |
|---|---|
| `VirtioPkg/Include/IndustryStandard/Virtio.h`     | `OvmfPkg/Include/IndustryStandard/Virtio.h` |
| `VirtioPkg/Include/IndustryStandard/Virtio095.h`  | `OvmfPkg/Include/IndustryStandard/Virtio095.h` |
| `VirtioPkg/Include/IndustryStandard/Virtio10.h`   | `OvmfPkg/Include/IndustryStandard/Virtio10.h` |
| `VirtioPkg/Include/IndustryStandard/VirtioBlk.h`  | `OvmfPkg/Include/IndustryStandard/VirtioBlk.h` |
| `VirtioPkg/Include/Protocol/VirtioDevice.h`       | `OvmfPkg/Include/Protocol/VirtioDevice.h` |
| `VirtioPkg/Include/Library/VirtioLib.h`           | `OvmfPkg/Include/Library/VirtioLib.h` |
| `VirtioPkg/Include/Library/PciCapLib.h`           | `OvmfPkg/Include/Library/PciCapLib.h` |
| `VirtioPkg/Include/Library/PciCapPciIoLib.h`      | `OvmfPkg/Include/Library/PciCapPciIoLib.h` |
...
1. Diff the VirtioPkg copy of a file against the same file at the recorded commit
   in `tianocore/edk2` to confirm it is still verbatim (aside from the INF
   `[Packages]` retarget).
