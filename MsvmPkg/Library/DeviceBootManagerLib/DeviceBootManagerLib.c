...
typedef BOOLEAN (*FILTER_ROUTINE)(
...
FilterHandles (
  ...
  UINTN           *HandleCount,
  FILTER_ROUTINE  KeepHandleFilter
  )
{
  UINTN                     Index;
  EFI_DEVICE_PATH_PROTOCOL  *DevicePath;
  ...
  *PlatformConsoles = NULL;
  ...
  if (ConsoleIn != NULL) {
    Status = gBS->HandleProtocol (
                    ConsoleIn,
                    &gEfiDevicePathProtocolGuid,
                    (void **)&(gPlatformConsoles[0].DevicePath)
                    );                                               // device path for ConIn
    if (EFI_ERROR (Status)) {
