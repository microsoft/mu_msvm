  ...
  )
{
  ...
  if ((EntryCount == 0) || ((DataLen % sizeof (MCFG_ALLOCATION_ENTRY)) != 0)) {
    DEBUG ((
      DEBUG_ERROR,
      "PCIe: PciSegmentInfoLib: Invalid MCFG data (len=%u, entry_size=%u)\n",
      DataLen,
      (UINT32)sizeof (MCFG_ALLOCATION_ENTRY)
      ));
    *Count = 0;
    return NULL;
  }
  ...
  for (i = 0; i < SegIdx; i++) {
    DEBUG ((
      DEBUG_INFO,
      "PCIe:   SegmentInfo[%u]: Seg=%u ECAM=%016lx Bus=%u..%u\n",
      i,
      mSegmentInfo[i].SegmentNumber,
      mSegmentInfo[i].BaseAddress,
      mSegmentInfo[i].StartBusNumber,
      mSegmentInfo[i].EndBusNumber
      ));
  }
