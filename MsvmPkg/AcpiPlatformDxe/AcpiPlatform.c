...
typedef EFI_STATUS (*INIT_ROUTINE)(
...
--*/
{
  ...
  while (hob != NULL) {
    ...
    DEBUG ((
      DEBUG_INFO,
      "Installing VMM-provided ACPI table: %.4a (0x%08x)\n",
    ...
    if (EFI_ERROR (status)) {
      DEBUG ((
        DEBUG_ERROR,
        "Failed to install VMM replacement table 0x%08x: %r\n",
        table->Signature,
        status
        ));
      return status;
...
--*/
{
  ...
  for (instance = 0; ; instance += 1) {
    ...
    if (AcpiSignatureHasReplacementHob (currentTable->Signature)) {
      DEBUG ((
        DEBUG_INFO,
        "Skipping FV table %.4a (0x%08x): VMM replacement provided\n",
        (CHAR8 *)&currentTable->Signature,
