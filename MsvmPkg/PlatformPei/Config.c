...
--*/
{
  ...
  if (maximumFunction >= CPUID_FUNCTION_EXTENDED_ADDRESS_SPACE_SIZES) {
    AsmCpuid (
      CPUID_FUNCTION_EXTENDED_ADDRESS_SPACE_SIZES,
    ...
    DEBUG ((
      DEBUG_WARN,
      "Can't query CPUID so defaulting address width to %u bits\n",
      minimumAddressWidth
      ));
  ...
  if ((regValue & 0xF) < 7) {
    ...
    DEBUG ((
      DEBUG_WARN,
      "Invalid D_AA64MMFR0_EL1.PARange so defaulting address width to %u bits\n",
      minimumAddressWidth
      ));
    physicalAddressWidth = minimumAddressWidth;
  }
  ...
  if (physicalAddressWidth < minimumAddressWidth) {
    DEBUG ((
      DEBUG_WARN,
      "Increasing address width from %u to %u\n",
      physicalAddressWidth,
      minimumAddressWidth
      ));
    physicalAddressWidth = minimumAddressWidth;
  ...
  if (physicalAddressWidth > maximumAddressWidth) {
    DEBUG ((
      DEBUG_WARN,
      "Reducing address width from %u to %u\n",
      physicalAddressWidth,
      maximumAddressWidth
      ));
  ...
  )
{
  ...
  do {
    switch (*cursor) {
      // UINT8 sized Type always at start of struct
      ...
      case EFI_ACPI_6_2_IO_APIC:
        ...
        DEBUG ((
          DEBUG_VERBOSE,
          "    IOAPIC Type %x Len %02x IoApicId %02x IoApicAddress %02x\n",
          madtIoApic->Type,
          madtIoApic->Length,
          madtIoApic->IoApicId,
          madtIoApic->IoApicAddress
          ));
      ...
      case EFI_ACPI_6_2_LOCAL_APIC_NMI:
        ...
        DEBUG ((
          DEBUG_VERBOSE,
          "    APIC NMI Type %x Len %02x Flags %02x AcpiProcessorUid %02x LocalApicLint %x\n",
          madtApicNmi->Type,
          madtApicNmi->Length,
          madtApicNmi->Flags,
          madtApicNmi->AcpiProcessorUid,
      ...
      case EFI_ACPI_6_2_INTERRUPT_SOURCE_OVERRIDE:
        ...
        DEBUG ((
          DEBUG_VERBOSE,
          "    Interrupt Source Override Type %x Len %02x Flags %02x Source %02x GlobalSystemInterrupt %x\n",
          madtOverride->Type,
          madtOverride->Length,
          madtOverride->Flags,
          madtOverride->Source,
      ...
      case EFI_ACPI_6_2_PROCESSOR_LOCAL_APIC:
        ...
        DEBUG ((
          DEBUG_VERBOSE,
          "    APIC Type %x Len %02x Flags %02x ApicId %02x\n",
          madtApic->Type,
          madtApic->Length,
          madtApic->Flags,
          madtApic->ApicId
      ...
      case EFI_ACPI_6_2_PROCESSOR_LOCAL_X2APIC:
        ...
        DEBUG ((
          DEBUG_VERBOSE,
          "   X2APIC Type %x Len %02x Flags %02x X2ApicId %02x\n",
          madtX2Apic->Type,
          madtX2Apic->Length,
          madtX2Apic->Flags,
          madtX2Apic->X2ApicId
      ...
      case EFI_ACPI_6_2_GICD:
        ...
        DEBUG ((
          DEBUG_VERBOSE,
          "   GICD Type %x Len %02x GicId %02x PhysicalBaseAddress %02x\n",
          madtGicd->Type,
          madtGicd->Length,
          madtGicd->GicId,
          madtGicd->PhysicalBaseAddress
          ));
      ...
      case EFI_ACPI_6_2_GIC:
        ...
        DEBUG ((
          DEBUG_VERBOSE,
          "   GICD Type %x Len %02x Flags %02x AcpiProcessorUid %02x\n",
          madtGicc->Type,
          madtGicc->Length,
          madtGicc->Flags,
          madtGicc->AcpiProcessorUid
          ));
  ...
  do {
    switch (*cursor) {
      ...
      case EFI_ACPI_6_2_PROCESSOR_LOCAL_APIC_SAPIC_AFFINITY:
        ...
        DEBUG ((
          DEBUG_VERBOSE,
          "    APIC Type %x Len %02x Flags %02x ApicId %02x Dom %x\n",
          sratApic->Type,
          sratApic->Length,
          sratApic->Flags,
          sratApic->ApicId,
      ...
      case EFI_ACPI_6_2_PROCESSOR_LOCAL_X2APIC_AFFINITY:
        ...
        DEBUG ((
          DEBUG_VERBOSE,
          "   X2APIC Type %x Len %02x Flags %02x X2ApicId %02x Dom %x\n",
          sratX2Apic->Type,
          sratX2Apic->Length,
          sratX2Apic->Flags,
          sratX2Apic->X2ApicId,
      ...
      case EFI_ACPI_6_2_GICC_AFFINITY:
        ...
        DEBUG ((
          DEBUG_VERBOSE,
          "   GICC Type %x Len %02x Flags %02x ProcessorUid %02x Dom %x\n",
          sratGicc->Type,
          sratGicc->Length,
          sratGicc->Flags,
          sratGicc->AcpiProcessorUid,
      ...
      case EFI_ACPI_6_2_MEMORY_AFFINITY:
        ...
        DEBUG ((
          DEBUG_VERBOSE,
          "    MEM  Type %x Len %02x Flags %02x Base % 14lx Len % 14lx "
          "Dom %x\n",
          sratMem->Type,
          sratMem->Length,
          sratMem->Flags,
...
--*/
{
  ...
  if (Header->Length % 8 != 0) {
    DEBUG ((
      DEBUG_ERROR,
      "Structure Type 0x%x was length 0x%x, not aligned to 8 bytes.\n",
      Header->Type,
      Header->Length
      ));
  ...
  if (Header->Length <= sizeof (UEFI_CONFIG_HEADER)) {
    DEBUG ((
      DEBUG_ERROR,
      "Structure Type 0x%x was length 0x%x, and smaller than the header size and information.\n",
      Header->Type,
      Header->Length
      ));
    return EFI_INVALID_PARAMETER;
  ...
  if ((expectedLength != 0) && (Header->Length != expectedLength)) {
    DEBUG ((
      DEBUG_ERROR,
      "Structure Type 0x%x was length 0x%x, expected Length %x\n",
      Header->Type,
      Header->Length,
      expectedLength
      ));
...
--*/
{
  ...
  for (UINT64 i = 1; i < configCount->TotalStructureCount; i++) {
    ...
    if (calculatedConfigSize > configCount->TotalConfigBlobSize) {
      DEBUG ((
        DEBUG_ERROR,
        "Config offset of 0x%x is greater than the actual size of 0x%x\n",
