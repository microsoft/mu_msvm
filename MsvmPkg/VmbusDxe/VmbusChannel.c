...
--*/
{
  ...
  status = VmbusRootGetFreeGpadl (
             channelContext->RootContext,
             &Gpadl->GpadlHandle
             );
  if (EFI_ERROR (status)) {
  ...
  receiveMessage = NULL;
  status         = VmbusRootWaitForGpadlResponse (
                     channelContext->RootContext,
                     Gpadl->GpadlHandle,
                     &receiveMessage
                     );
...
--*/
{
  ...
  return VmbusChannelOpenChannel (
...
--*/
{
  ...
  CopyMem (
    &ChannelContext->DevicePath.VmbusChannelNode,
    &gVmbusChannelNode,
