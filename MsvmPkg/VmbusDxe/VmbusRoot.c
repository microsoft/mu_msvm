...
{
  { TRUE,  { 0xba6163d9, 0x04a1, 0x4d29, { 0xb6, 0x05, 0x72, 0xe2, 0xff, 0xb1, 0xdc, 0x7f }
    }
  },                                                                                            // StorvscDxe
  { TRUE,  { 0xf8615163, 0xdf3e, 0x46c5, { 0x91, 0x3f, 0xf2, 0xd2, 0xf9, 0x65, 0xed, 0xe  }
    }
  },                                                                                            // NetvscDxe
  { TRUE,  { 0x44c4f61d, 0x4444, 0x4400, { 0x9d, 0x52, 0x80, 0x2e, 0x27, 0xed, 0xe1, 0x9f }
    }
  },                                                                                            // VpcivscDxe
  { FALSE, { 0xda0a7802, 0xe377, 0x4aac, { 0x8e, 0x77, 0x05, 0x58, 0xeb, 0x10, 0x73, 0xf8 }
    }
  },                                                                                            // VideoDxe
  { FALSE, { 0xc376c1c3, 0xd276, 0x48d2, { 0x90, 0xa9, 0xc0, 0x47, 0x48, 0x07, 0x2c, 0x60 }
    }
  },                                                                                            // VmbfsDxe
  { FALSE, { 0xf912ad6d, 0x2b17, 0x48ea, { 0xbd, 0x65, 0xf9, 0x27, 0xa6, 0x1c, 0x76, 0x84 }
    }
  }                                                                                             // SynthKeyDxe
};

EFI_GUID  gVmbfsChannelGuid =  {
...
*/
{
  ...
  DEBUG ((
    EFI_D_INFO,
    "%a(%d) channelContext = %p ChannelId 0x%x\n",
    __func__,
    __LINE__,
    ChannelContext,
...
**/
{
  ...
  while (!IsListEmpty (&RootContext->HotMessageList)) {
    hotMessage = BASE_CR (
                   RemoveEntryList (GetFirstNode (&RootContext->HotMessageList)),
                   VMBUS_HOT_MESSAGE,
                   Link
                   );
...
**/
{
      ...
      (hotMessage->Message.OfferChannel.ChildRelId >= VMBUS_MAX_CHANNELS))
  {
    DEBUG ((
      EFI_D_ERROR,
      "--- %a: invalid offer message: %#x (size %d), rel ID %d",
      __func__,
...
**/
{
  ...
  DEBUG ((
    EFI_D_WARN,
    "%a (%d) orphaned %d GPADLs (IsolationArchitecture=%d)\n",
    __func__,
    __LINE__,
    orphanedGpadlCount,
    GetIsolationType ()
