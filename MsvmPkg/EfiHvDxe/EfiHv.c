...
//
// Internal DMA pinning helpers. These functions implement host query,
// always-pinned subrange skipping, explicit pin tracking, and rollback for
// the EFI_HV_IVM_PROTOCOL PinAddressRange/UnpinAddressRange entry points.
//
// TODO: Move this pinning implementation into EfiHvPinning.c with an
// EfiHvInternal.h private header once this behavioral change has settled.
//
...
--*/
{
    ...
    for (;;)
    {
        ...
        if (rangesProcessedThisCall > rangesInCurrentCall)
        {
            FAIL_FAST_UNEXPECTED_HOST_BEHAVIOR();
        }
        ...
        if (pagesProcessedThisCall > PageCount)
        {
            FAIL_FAST_UNEXPECTED_HOST_BEHAVIOR();
        }
        else
