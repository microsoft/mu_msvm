...
--*/
{
  ...
  status = pDevice->Emcl->SetReceiveCallback (
                            pDevice->Emcl,
                            SynthKeyChannelReceivePacketCallback,
                            pDevice,
  ...
  status = pDevice->Emcl->StartChannel (
                            pDevice->Emcl,
...
--*/
{
  ...
  //
  status = pDevice->Emcl->SendPacket (
                            pDevice->Emcl,
                            Message,
                            MessageSize,
  ...
  if (EFI_ERROR (status)) {
    DEBUG ((
      EFI_D_WARN,
      "--- %a: failed to send the message (type %d, size %d) --%r \n",
      __func__,
      Message->MessageType,
      MessageSize,
      status
...
--*/
{
  ...
  switch (message->MessageType) {
    ...
    default:
      DEBUG ((
        EFI_D_WARN,
        "--- %a: unknown message type (type %d, size %d) \n",
        __func__,
        message->MessageType,
        BufferLength
        ));
